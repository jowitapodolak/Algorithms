...
\IfFileExists{vc.tex}{\input{vc.tex}}{\newcommand\GITAuthorDate{no git info found}\newcommand\GITAbrHash{-}}  % For version control \input{vc.tex}

\title{Runsort}
\author{Thore Husfeldt (modified by Riko Jacob)}
...
\begin{enumerate}
\item In java, write a class \texttt{Runsort} that includes a method \texttt{public static void sort(Comparable[] a)}.
  In python, write a function \texttt{sort(a)} that relies solely on comparison of the elements in the array (python list) \texttt{a}.
\item You must implement the above algorithm, in particular its way of manipulating arrays as arrays and not as lists.
\item Don't implement any of the other sorting algorithms discussed in class.
...
Here, the \texttt{sort} and \texttt{diff} functionality of your operating system can be a big help in your debugging and testing efforts.
...
\begin{enumerate}
  ...
  \item A brief report in PDF, telling us if the code seems to sort, what you've tested it on, and a brief description of your extension.
    ...
    Include a timestamp and submission number from your successful submission of code to codeJudge.
...
by Alice Cooper and Bob Marley\sidenote{Complete the report by filling
  ...
  We uploaded the handed in code at [14-02-2018 13:46] as submission \#886428 on  codeJudge.
  ...
  Our algorithm is twice as fast on the \texttt{*run*} in comparison to the \texttt{*rnd*} instances.
